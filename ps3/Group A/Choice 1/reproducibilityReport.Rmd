...
table(d$condition, d$exchangeinfo)
```
...
```{r}
# reproduce the above results here
table <- table(d$condition, d$exchangeinfo)
chisq.test(table, correct=F) # results would be slightly different if run with Yates' continuity correction
```
...
> Markdown script was very helpful, as well as the specific instructions what to replicate; unintuitive was that R's automatic correction in the chi-squared test turned out to be the reason why I initially found different inferential test results
